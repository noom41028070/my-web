...
FirehahaPlugins.register({
  ...
  setup(api) {
    ...
    function readerRuntime() {
      ...
      /*
       * 保存這一次 applyAdventure() 剛產生、但尚未真正塞進 DOM 的 HTML。
       * 自動骰 Hook 會先把 [自動骰] 轉成 .fh-auto-dice-result，
       * 對抗骰必須能直接從這份 HTML 讀值，否則同一次渲染時只查 DOM
       * 會拿到舊畫面，導致永遠停在「等待」而算不出左勝 / 右勝。
       */
      ...
      function findAutoDiceFromCurrentHtml(
        name
      ) {
        const target =
          String(name || "")
      ...
      function findAutoDice(
        ...
        if (exposed) {
          ...
          if (target) {
            if (exposed[target] != null) {
              raw = exposed[target];
            } else if (exposed["name:" + target] != null) {
              raw = exposed["name:" + target];
            }
          } else {
            const values = Object.values(exposed);
            if (values.length) raw = values[values.length - 1];
          }
        ...
        /*
         * 同一次 applyAdventure() 內，自動骰雖然已經被轉成 HTML，
         * 但畫面 DOM 還沒更新。先直接從本次 HTML 字串讀取，
         * 這是避免「永遠等待、算不出左勝」的關鍵。
         */
        const fromCurrentHtml =
          findAutoDiceFromCurrentHtml(
            target
          );

        if (fromCurrentHtml) {
          return fromCurrentHtml;
        }


        /*
         * 最後才從目前閱讀器 DOM 找（作為重畫 / 舊版相容保底）。
         */
        const elements =
          Array.from(
            document.querySelectorAll(
      ...
      //
      // 重要：
      // 其他較早掛入 applyAdventure 的通用 [如果] 外掛，
      // 可能會先看到 [如果:對抗:...]，因為它不認得「對抗」
      // 而直接走 [否則]，造成之後即使算出左勝也永遠只顯示失敗。
      //
      // 所以在交給舊 applyAdventure 前，先把本模組自己的
      // 對抗條件整塊暫存成安全 token；舊流程跑完後再還原，
      // 最後才由 processOpposedIf() 判斷。
      // =====================================================

      function protectOpposedIfBlocks(
        source
      ) {
        const blocks = [];
      ...
      applyAdventure =
        function(page) {
          /*
           * 先保護 [如果:對抗:...] / [如果:對抗差值:...]，
           * 避免前面的通用 if-block 把未知條件提早判成 false。
           */
          const clonedPage =
            Object.assign(
              {},
          ...
          /*
           * 這時自動骰 Hook 已可能把 [自動骰] 轉成結果卡，
           * 但 Reader 尚未把 html 寫進 DOM。
           * 對抗骰直接讀這份本次 HTML，才能在同一輪就得到左右數值。
           */
          currentRenderedHtml =
            String(html || "");
    ...
    const removeTransform =
      api.registerReaderTransform(
        ...
        function(
          ...
          /*
           * V1.2：優先掛在 renderAdventure() 宣告前。
           * 這個位置位於 Reader 主程式作用域內，而且不會和
           * dice-auto-if 對 }loadState(); 的替換互相搶掛點。
           */
          const renderMarker =
            "function renderAdventure(){";

          if (output.includes(renderMarker)) {
            ...
            /*
             * 相容舊 Reader：如果還找得到原本掛點，再用舊方式。
             */
            const loadMarker =
              "}loadState();";

            if (output.includes(loadMarker)) {
              output = output.replace(
                loadMarker,
                "}" + runtimeSource + "loadState();"
              );
            } else {
              /*
               * 最後保底仍可執行，但會警告掛載較晚。
               */
              console.warn(
                "[Opposed Dice] 找不到 Reader 內部掛點，改用 body fallback"
              );
