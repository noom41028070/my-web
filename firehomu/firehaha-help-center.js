  ...
  setup(api) {
    ...
    const entries = [
      ...
      },
      {
        ...
        summary: tri("頁面顯示時自動擲 D4、D6、D20 或 D100，可加減固定值並命名結果；接著用 [如果:自動骰…] 或 [如果:骰子…] 顯示不同正文。", "Roll D4, D6, D20 or D100 automatically when the page opens, with an optional modifier and result name. Use an if block to branch on the result.", "ページ表示時にD4／D6／D20／D100を自動で振り、結果を条件分岐に使用します。"),
        examples: [ex("[自動骰:d20+2|陷阱]"), ex("[如果:自動骰:陷阱>=15]成功避開陷阱[否則]踩中了陷阱[/如果]"), ex("[如果:骰子:力量>=18]力量壓倒對手[/如果]")]
      },
      {
